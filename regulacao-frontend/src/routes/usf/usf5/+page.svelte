...
<svelte:head>
  <title>Pendentes â€” {usfOrigem}</title>
</svelte:head>

<div class="flex min-h-screen bg-gray-100">
  <RoleBasedMenu activePage="/usf/usf5" />

  <div class="flex-1 flex flex-col">
    <header class="bg-emerald-700 text-white shadow p-4 flex items-center justify-between">
      <h1 class="text-xl font-semibold">Pacientes Pendentes â€” {usfOrigem}</h1>
      <UserMenu />
    </header>
        ...
        {:else}
          ...
          {#if totalElements === 0}
            <p class="text-center text-gray-500 py-10">
              {#if termo.trim()}
                Nenhuma solicitaÃ§Ã£o encontrada para "{termo}".
              {:else}
                Nenhuma solicitaÃ§Ã£o pendente para {usfOrigem} no momento.
              {/if}
            </p>
          {:else}
            <ul class="space-y-4">
